...
public class Slime : MonoBehaviour
{
    ...
    public void Die()
    {
        if (!isDead)
        {
            ...
            GameManager.Inst.MainPlayer.LifeTime += 2.0f;
    ...
    private IEnumerator DieProcess()
    {
        ...
        float dissolveNomalize = 1 / dissolveTime;

        while(timeElipsed < dissolveTime)
        {
            timeElipsed += Time.deltaTime;
            mainMat.SetFloat("_DissolveFade", 1 - (timeElipsed * dissolveNomalize));
            yield return null;
        }
        onDead?.Invoke(this);
    ...
    void DrawPath()
    {
        if (showPath && path != null && path.Count > 1)  // 최소 2개의 위치는 필요함
        {
            ...
            foreach (var pos in path)
            {
                ...
                line.SetPosition(index, new(worldPos.x,
                    worldPos.y, 1));    // 경로를 이용해서 라인랜더러가 그려질 위치 결정
                index++;
            }
        }
        else
