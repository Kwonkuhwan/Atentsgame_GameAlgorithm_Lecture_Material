...
public class SubMapManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        spawnRequests = new Queue<Spawner>();   // 몬스터 생성 요청 기록 큐
    ...
    /// 스폰 가능한 지역 목록 만들기(Gridmap 랩핑함수)
    /// </summary>
    /// <param name="spawner">생성할 스포너</param>
    /// <returns>스폰 가능한 그리드 좌표의 목록</returns>
    private List<Vector2Int> SpawnablePostions(Spawner spawner)
    {
        //List<Vector2Int> SpawnablePostions(Vector2Int min, Vector2Int max)를 사용하기 위해 최소값 최대값 구하기
        ...
        // -1을 한 이유는 월드 좌표를 그리드 좌표는 변경할 때 1칸 만큼 
    ...
    List<Vector2Int> ShuffleList(List<Vector2Int> list)
    {
        ...
        // 교환 대상 노드의 오른쪽에 있는 나머지 노드들 중 임의로 하나를 골라 교환 대상 노드를 위치를 교환한다.
        // 교환 대상 노드의 오른쪽 옆에 있는 노드를 새로운 교환 대상 노드로 지정한다.
        // 교환 대상 노드가 제 오른쪽 노드가 될 때까지 반복한다.
